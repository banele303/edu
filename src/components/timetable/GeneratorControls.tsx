...
const GeneratorControls = ({
  ...
  const loadingData = convexClasses === undefined || convexYears === undefined;

  useEffect(() => {
    if (convexYears && convexYears.length > 0 && !selectedYear) {
      // Auto-select current year
      const current = convexYears.find((y: any) => y.isCurrent);
      if (current?._id) setSelectedYear(current._id);
    }
  }, [convexYears, selectedYear]);
